...
def retrieve_and_decrypt():
    try:
        ...
        logging.debug(f"Encrypted data (hex): {encrypted_data_hex}")
        logging.debug(f"Data hash from ThingSpeak: {data_hash}")

        if encrypted_data_hex and data_hash:
            encrypted_data = bytes.fromhex(encrypted_data_hex)
            logging.debug(f"Encrypted data (bytes): {encrypted_data}")
